...
Furthermore, for any index 1 &lt; i ≤ k we have that y(p<sub>i</sub>) &gt; y(p<sub>i-1</sub>) if i is even and y(p<sub>i</sub>) &lt; y(p<sub>i-1</sub>) if i is odd. To paraphrase, after translating and rotating the points we have that the x coordinates of the points are strictly increasing, the y coordinates of all but the two endpoints are strictly positive, and the difference between y coordinates of successive points alternates between positive and negative.</p>
<p>
2)<br />
No blossoms may be contained strictly in the interior of the crown.</p>
<p>
You can find some pictures of crowns following the sample data for this problem.</p>
<p>
Subject to these restrictions, the Chef wants to draw the crown with the largest possible area. Thankfully, his job is made slightly simpler since no three blossoms are collinear.</p>
<h3>Input</h3>
...
<p>
As promised in the problem statement, no three blossoms will be collinear. Also, no two blossoms will have both the same X and Y coordinates.</p>
<h3>Output</h3>

<p>
The output for each test case is a single floating point number with 1 decimal of precision indicating the largest possible area of a crown drawn according to the restrictions above.</p>
<h3>Example</h3>
<pre>
<b>Input:</b>
4
